...
const HobbyCard: React.FC<HobbyCardProps> = ({ hobby, index }) => {
  ...
  const currentHero = images[activeImageIndex] || images[0] || '';

  // Thumbnail list
  const otherImages = images.map((img, idx) => ({ img, idx }));
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10">
        ...
        {/* Discord-style Multi-Image Interactive Gallery */}
        {images.length > 0 && (
          <div className="relative w-full mb-5">
            {images.length === 1 ? (
              ...
              <div className="grid grid-cols-3 gap-2 aspect-[16/10] overflow-hidden rounded-lg bg-light-surface-raised dark:bg-dark-surface-raised border border-light-border/60 dark:border-dark-border/60 p-1">
                {/* Primary Hero Image (Spans 2 columns) */}
                <div className="col-span-2 relative overflow-hidden rounded bg-light-surface dark:bg-dark-surface group/hero">
                  <img
                    src={currentHero}
                    alt={`${hobby.title} main preview`}
                ...
                {/* Side Stacked Thumbnail Column (Spans 1 column) */}
                <div className="flex flex-col gap-1.5 h-full overflow-y-auto [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
                  {otherImages.map(({ img, idx }) => {
                    ...
                    return (
                      <button
                        ...
                        className={`relative flex-1 min-h-[50px] overflow-hidden rounded border transition-all duration-200 cursor-pointer group/thumb ${
                          isSelected
                            ? 'border-terracotta ring-1 ring-terracotta/50 opacity-100 shadow-2xs'
                            : 'border-light-border/60 dark:border-dark-border/60 opacity-60 hover:opacity-100 hover:border-terracotta/50'
                        }`}
                        title={`View photo ${idx + 1}`}
                        aria-label={`View photo ${idx + 1} for ${hobby.title}`}
                      >
                        <img
                          src={img}
                        ...
                        />
                        {isSelected && (
                          <span className="absolute top-1 right-1 w-2 h-2 rounded-full bg-terracotta shadow-xs" />
                        )}
                      </button>
                    );
                  })}
            ...
            <div className="flex items-center justify-between text-[11px] text-light-ink-muted/80 dark:text-dark-ink-muted/80 mt-1.5 px-0.5 font-mono">
              <span className="text-[10px] opacity-70">Click thumbnails to preview</span>
...
export const HobbiesSection: React.FC = () => {
  ...
  const categories = Array.from(
    new Set(hobbies.map((h) => h.category).filter((c): c is string => Boolean(c)))
  );

  const filteredHobbies =
    activeCategory === 'all'
      ? hobbies
      : hobbies.filter((h) => h.category?.toLowerCase() === activeCategory.toLowerCase());
    ...
    <section id="hobbies" className="relative w-full overflow-hidden py-16 lg:py-24">
      ...
      <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 relative z-10">
        ...
        <div className="mb-8 sm:mb-10 pb-6 border-b border-light-border/70 dark:border-[#2D3039]/80">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div className="max-w-3xl">
              ...
              <p className="font-sans text-sm sm:text-base text-light-ink-muted dark:text-dark-ink-muted mt-3 font-light leading-relaxed">
                Bridging the physical and digital. Traditional joinery, tea ceremonies, sumi calligraphy, and custom keyboard engineering cultivate the patience, tactile rigor, and structural humility essential to resilient software systems.
              </p>
            </div>

            {/* Quick Status / Counter Badge */}
            <div className="hidden sm:flex items-center gap-2 px-3.5 py-2 rounded-lg bg-light-surface-card dark:bg-dark-surface border border-light-border dark:border-dark-border text-xs font-mono shrink-0 select-none shadow-2xs">
              <span className="w-2 h-2 rounded-full bg-terracotta animate-pulse" />
              <span className="text-light-ink-muted dark:text-dark-ink-muted uppercase tracking-wider">
                {hobbies.length} Pursuits in Practice
              </span>
            </div>
          </div>
          ...
          {categories.length > 1 && (
            <div className="flex flex-wrap items-center gap-2 mt-6 pt-4 border-t border-light-border/50 dark:border-dark-border/50">
              <button
                type="button"
                onClick={() => setActiveCategory('all')}
                className={`px-3 py-1.5 rounded-lg text-xs font-mono uppercase tracking-wider transition-all duration-200 cursor-pointer ${
                  activeCategory === 'all'
                    ? 'bg-terracotta/15 text-terracotta border border-terracotta/40 font-semibold shadow-xs'
                    : 'bg-light-surface-card dark:bg-dark-surface border border-light-border dark:border-dark-border text-light-ink-muted dark:text-dark-ink-muted hover:text-light-ink dark:hover:text-dark-ink'
                }`}
              >
                All Pursuits ({hobbies.length})
              </button>
              {categories.map((cat) => {
                const count = hobbies.filter((h) => h.category?.toLowerCase() === cat.toLowerCase()).length;
        ...
        {/* 2-Column Responsive Hobbies Grid */}
