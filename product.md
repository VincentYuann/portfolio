...
* **Admin Database Copilot**: Verified administrators can upload resumes, images, or text to have the AI generate safe PostgreSQL operations, preserving exact requested fields 1-to-1 while semantically synthesizing unspecified fields. Mutations execute via PostgreSQL RPC `public.execute_admin_sql` using Supabase Auth JWTs (zero management tokens `sbp_...` stored), triggering instant Supabase Realtime synchronization on the frontend.
* **Speech-to-Text Voice Dictation**: Verified administrators have access to a real-time microphone dictation button in the input dock (powered by `react-speech-recognition`), featuring live speech indicators and auto-appending transcribed speech directly into the prompt box.
* **3-Layer Security & Leak Protection**: Non-admin visitors receive role-isolated prompts without database schema leakage, and administrative SQL execution tools are completely excluded from non-admin sessions at both the Gemini tool-declaration and backend validation layers.
